...
def run_beam_search(sess, model, vocab, batch):
    ...
    enc_states, dec_in_state, query_vector, review_states = model.run_encoder(sess, batch)
    fused_review = None
    fused_attribute = None
    ...
    while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
        ...
        rr = model.decode_onestep(sess=sess, batch=batch, latest_tokens=latest_tokens, enc_states=enc_states,
                                  dec_init_states=states, prev_coverage=prev_coverage, fused_review=fused_review,
                                  review_states=review_states,
                                  query_vector=query_vector)
        (topk_ids, topk_log_probs, new_states, attn_dists, p_gens, new_coverage) = rr
