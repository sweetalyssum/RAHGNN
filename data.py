...
class Vocab(object):
  ...
  def __init__(self, vocab_file, vocab_emb_file, max_size):
    ...
    self.emb = []
    vocab_emb_f = open(vocab_emb_file, 'r')
    with open(vocab_file, 'r', encoding='utf8') as vocab_f:
      for line, emb in izip.iteration(vocab_f, vocab_emb_f):
        pieces = line.split()
        if len(pieces) != 2:
        ...
        self._id_to_word[self._count] = w
        self._count += 1
        emb_vector = json.loads(emb.strip())
        self.emb.append(emb_vector)
