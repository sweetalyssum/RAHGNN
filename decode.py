...
# from exp_uploader import append_results, Exp
# from rouge_server import eval_rouge as eval_rouge_server
...
class BeamSearchDecoder(object):
    ...
    def __init__(self, model, batcher, vocab, ckpt_dir=None, ckpt_name=None, saver=None, session=None):
        """Initialize decoder.
    ...
    def decode(self):
        ...
        if FLAGS.decode_rouge_server:
            self.decoded_list = []
            self.reference_list = []
            self.decoded_num_list = []
            self.reference_num_list = []
        while True:
            ...
            if batch is None:  # finished decoding dataset in single_pass mode
                ...
                if FLAGS.decode_rouge:
                    ...
                    try:
                        if FLAGS.decode_rouge_server:
                            eval_rouge_server(FLAGS.proj_name, FLAGS.exp_name, self._decode_dir_name, self.decoded_list,
                                              self.reference_list)
                            if FLAGS.lang == 'zh':
                                eval_rouge_server(FLAGS.proj_name, FLAGS.exp_name, self._decode_dir_name + 'NUM',
                                                  self.decoded_num_list, self.reference_num_list)
                        t0 = time.time()
                        ...
                        if FLAGS.lang == 'zh':
                            ...
                            final_metrics['rouge-1-f-num'] = num_results_dict['rouge_1_f_score']
                            final_metrics['rouge-2-f-num'] = num_results_dict['rouge_2_f_score']
                            final_metrics['rouge-L-f-num'] = num_results_dict['rouge_l_f_score']
                            final_metrics['rouge-1-r-num'] = num_results_dict['rouge_1_recall']
                            final_metrics['rouge-2-r-num'] = num_results_dict['rouge_2_recall']
                            final_metrics['rouge-L-r-num'] = num_results_dict['rouge_l_recall']
                            final_metrics['rouge-dict-num'] = num_results_dict
                        ...
                        sys.stderr.write('计算ROUGE出错 error %s \n' % e)
                        sys.stderr.flush()
                if FLAGS.decode_bleu:
                    try:
                        ...
                        sys.stderr.write('计算BLEU出错 error %s \n' % e)
                        sys.stderr.flush()
                break
            ...
            if FLAGS.single_pass:
                if FLAGS.decode_bleu:
                    self.write_for_rouge(original_abstract_sents, decoded_words, counter, num_dict)
                if FLAGS.decode_bleu:
    ...
    def write_for_rouge(self, reference_sents, decoded_words, ex_index, num_dict=None):
        ...
        if FLAGS.decode_rouge_server:
            self.reference_list.append(reference_sents)
            self.decoded_list.append(decoded_sents)
    ...
    def convert_to_num_sentence(self, sent, num_dict):
        num_sent = []
        for w in sent.split(' '):
